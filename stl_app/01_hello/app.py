...
st.markdown('สวัสดี! *Streamlit*')   # *ข้อความในนี้จะเอียง* , **ข้อความในนี้จะตัวหนา**
st.write('จากโค้ด', '`st.markdown("สวัสดี!")`')  # ` Bactig จะใส่กรอบให้ เหมือนเอาทุกตัวมาแสดง,คล้ายในโปรแกรม
# st.write(pd.DataFrame({
#     'first column': [1, 2, 3, 4],
#     'second column': [10, 20, 30, 40],
# }))
st.divider() #เหมือนขีดเส้น
...
# Fix : 4. https://docs.streamlit.io/library/api-reference/widgets
...
# Fix : 1. where to replace `perc_heads`?
list_of_means = []
for i in range(0, 1000):
    list_of_means.append(
        np.random.choice(binom_dist, 100, replace=True).mean())
...
# Fix : 2. how to setup limit of x-axis value to 0.0 - 1.0?
# Fix : 3. how to setup more bins, like 20 or 40?
st.pyplot(fig1)
